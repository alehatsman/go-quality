#!/usr/bin/env bash
# budget-status — print current state of the code-quality soft caps.
#
...
#
# Projects with extra caps (e.g. per-handler LOC, struct-field budgets) layer
# their own budget script on top; this is the shared baseline.
#
...
if command -v gocyclo >/dev/null 2>&1 || [ -x "$(go env GOPATH)/bin/gocyclo" ]; then
  ...
  echo "   gocyclo not installed — run scripts/install-tools.sh"
fi
echo
